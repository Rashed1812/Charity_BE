...
namespace Charity_BE.Controllers
{
    ...
    public class AdviceRequestController : ControllerBase
    {
        ...
        public AdviceRequestController(IAdviceRequestService adviceRequestService, INotificationService notificationService)
        {
            ...
            try
            {
                ...
                if (request.AdvisorId.HasValue)
                {
                    var notification = new NotificationCreateDTO
                    {
                        ...
                        Message = $"قام المستخدم {request.UserName} بإرسال طلب استشارة بعنوان: \"{request.Title}\" ضمن تخصص {request.ConsultationName}.",
        ...
        public async Task<ActionResult<ApiResponse<AdviceRequestDTO>>> ConfirmRequest(int id, [FromQuery] string advisorId)
        {
            try
            {
                ...
                var notification = new NotificationCreateDTO
                {
                    ...
                    Message = $"قام المستشار {request.AdvisorName} بتأكيد موعد الاستشارة بعنوان \"{request.Title}\"",
                    Type = NotificationType.General
                };
                await _notificationService.AddNotificationAsync(notification);
            ...
            [FromBody] CompleteRequestDTO completeRequestDto)
        {
            ...
            try
            {
                ...
                var notification = new NotificationCreateDTO
                {
                    ...
                    Message = $"تم الانتهاء من جلسة الاستشارة الخاصة بك بعنوان \"{request.Title}\".\nيرجى إرسال تقييمك عبر مركز الشكاوى والمقترحات.",
                    Type = NotificationType.General
                };
                await _notificationService.AddNotificationAsync(notification);
